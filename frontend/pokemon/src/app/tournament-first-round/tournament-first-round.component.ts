...
@Component({
  selector: 'app-tournament-first-round',
  templateUrl: './tournament-first-round.component.html',
  styleUrls: ['./tournament-first-round.component.css']
})
export class TournamentFirstRoundComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.num_of_match_finished = Number(localStorage.getItem("t"+this.id+"r1-matches_finished"));
    $('#progress_bar').width(Math.round(this.num_of_match_finished * 100 / 32) + "%").attr('aria-valuenow', this.num_of_match_finished);
  ...
  find_tournament_by_id(id: Number): void {
    this.service.find_tournament_by_id(id).subscribe(data => {
      ...
      this.service.find_matches_by_type(id, "r1").subscribe(data => {
  ...
  add_new_match(index: number, matches_: Match[], pokemon: number[], size: number): void {
    ...
    if (m.result == 2) {
      ...
      this.service.update_points_tournament(this.id, m.idPokemon1, 3).subscribe(data => {
        if (data['ok'] == 'no') {
        ...
        if (index < size) {
          ...
          this.service.get_match_max_id().subscribe(data => {
            ...
            for (let i = 0; i < number_of_matches; i++) {
              ...
              this.service.add_new_match(match_id, 0, this.id, p1, p2, "r2").subscribe(data => {
                if (data['ok'] == 'no') {
                  alert("Something went wrong");
                  return;
                }
                ...
                if (i == number_of_matches - 1) {
                  this.router1.navigate(['/tournament_second_round/' + this.id]);
                }
              })
      ...
      this.service.update_points_tournament(this.id, m.idPokemon2, 3).subscribe(data => {
        if (data['ok'] == 'no') {
          alert("Something went wrong");
          return;
        }
