...
@Component({
  selector: 'app-add-fast-attack',
  templateUrl: './add-fast-attack.component.html',
  styleUrls: ['./add-fast-attack.component.css']
})
export class AddFastAttackComponent implements OnInit {
  ...
  add_new_fast_attack(): void{
      ...
      this.service.find_fast_attack_by_name(this.name).subscribe(data=>{
        let fast_attack = JSON.parse(JSON.stringify(data));
        if(fast_attack != null){
          this.message = "Fast Attack already exists.";
          return;
        }else{
          this.service.get_fast_attack_max_id().subscribe(data=>{
            ...
            this.service.add_fast_attack(id, this.name, this.type, this.damage, this.energy, this.duration)
            .subscribe(data=>{
          if(data['ok'] == 'yes'){
            this.message = "Fast Attack has been added.";
            this.added = true;
            this.name=null;
            this.type = 0;
            this.duration = null;
