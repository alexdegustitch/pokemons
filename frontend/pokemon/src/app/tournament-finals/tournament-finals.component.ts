...
declare var $: any;
@Component({
  selector: 'app-tournament-finals',
  templateUrl: './tournament-finals.component.html',
  styleUrls: ['./tournament-finals.component.css']
})
export class TournamentFinalsComponent implements OnInit {
  ...
  find_tournament_by_id(id: Number): void {
    this.service.find_tournament_by_id(id).subscribe(data => {
      ...
      this.service.find_matches_by_type(id, "f").subscribe(data => {
  ...
  add_pokemon_points(index: number, pokemon: Pokemon[], size: number): void {

    this.service.find_points_tournament(Number(this.id), Number(pokemon[index].id)).subscribe(data => {
      let points: PointsTournament;
      points = JSON.parse(JSON.stringify(data))[0];
      localStorage.setItem("points", JSON.stringify(points));
      this.service.find_points_for_pokemon(Number(pokemon[index].id)).subscribe(data => {
        let old_points: Points;
        old_points = JSON.parse(JSON.stringify(data))[0];
        let new_points: number;
        new_points = points.points + old_points.points;
        //alert("old " + old_points.points + " points " + points.points + " new " + new_points);
        this.service.update_points(Number(pokemon[index].id), Number(new_points)).subscribe(data => {
          if (data['ok'] == 'no') {
          ...
          index++;
          if (index == size) {
            this.router1.navigate(['/table_points']);
          } else {
            this.add_pokemon_points(index, pokemon, size);
          }
        })
      })
    })
  ...
  next_stage(): void {
      ...
      if (this.matches[0].result == 1) {
        this.service.update_points_tournament(this.id, this.matches[0].idPokemon1, 20).subscribe(data => {
          if (data['ok'] == 'no') {
            alert("Something went wrong");
            return;
          }
          this.service.tournament_add_1_place(this.id, this.match_details[0].idPokemon1, this.match_details[0].namePokemon1).subscribe(data => {
            if (data['ok'] == 'no') {
              alert("Something went wrong");
              return;
            }
            this.service.update_points_tournament(this.id, this.matches[0].idPokemon2, 17).subscribe(data => {
              if (data['ok'] == 'no') {
                alert("Something went wrong");
                return;
              }
              this.service.tournament_add_2_place(this.id, this.match_details[0].idPokemon2, this.match_details[0].namePokemon2).subscribe(data => {
                if (data['ok'] == 'no') {
                  alert("Something went wrong");
                  return;
                }
                ...
                this.service.get_all_pokemon().subscribe(data => {
                  ...
                  this.add_pokemon_points(0, pokemon, pokemon.length);
                })
        ...
        this.service.update_points_tournament(this.id, this.matches[0].idPokemon2, 20).subscribe(data => {
          if (data['ok'] == 'no') {
            alert("Something went wrong");
            return;
          }
          this.service.tournament_add_1_place(this.id, this.match_details[0].idPokemon2, this.match_details[0].namePokemon2).subscribe(data => {
            if (data['ok'] == 'no') {
              alert("Something went wrong");
              return;
            }
            this.service.update_points_tournament(this.id, this.matches[0].idPokemon1, 17).subscribe(data => {
              if (data['ok'] == 'no') {
                alert("Something went wrong");
                return;
              }
              this.service.tournament_add_2_place(this.id, this.match_details[0].idPokemon1, this.match_details[0].namePokemon1).subscribe(data => {
                if (data['ok'] == 'no') {
                  alert("Something went wrong");
                  return;
                }
