...
declare var $: any;
@Component({
  selector: 'app-tournament-third-round',
  templateUrl: './tournament-third-round.component.html',
  styleUrls: ['./tournament-third-round.component.css']
})
export class TournamentThirdRoundComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.num_of_match_finished = Number(localStorage.getItem("t"+this.id+"r3-matches_finished"));
    $('#progress_bar').width(Math.round(this.num_of_match_finished * 100 / 8) + "%").attr('aria-valuenow', this.num_of_match_finished);
  ...
  find_tournament_by_id(id: Number): void {
    this.service.find_tournament_by_id(id).subscribe(data => {
      ...
      this.service.find_matches_by_type(id, "r3").subscribe(data => {
  ...
  add_new_match(index: number, matches_: Match[], pokemon: number[], size: number): void {
    ...
    if (m.result == 2) {
      ...
      this.service.update_points_tournament(this.id, m.idPokemon1, 7).subscribe(data => {
        if (data['ok'] == 'no') {
        ...
        if (index < size) {
          ...
          this.service.get_match_max_id().subscribe(data => {
            ...
            for (let i = 0; i < number_of_matches; i++) {
              ...
              this.service.add_new_match(match_id, 0, this.id, p1, p2, "q").subscribe(data => {
                if (data['ok'] == 'no') {
                  alert("Something went wrong");
                  return;
                }
                ...
                if (i == number_of_matches - 1) {
                  this.router1.navigate(['/tournament_quarterfinals/' + this.id]);
                }
              })
      ...
      this.service.update_points_tournament(this.id, m.idPokemon2, 7).subscribe(data => {
        if (data['ok'] == 'no') {
          alert("Something went wrong");
          return;
        }
                            ...
                            this.service.find_main_attack_by_id(main_attack.idAttack).subscribe(data => {
                              ...
                              this.service.find_pokemon_details_by_pokemon_and_type(match.idPokemon1, main_attack2.type).subscribe(data => {
                                ...
                                this.service.update_match(match.id, Math.round(hp_left1), Math.round(hp_left2), res, fast_attack1.id, fast_attack2.id, main_attack1.id, main_attack2.id).subscribe(data => {
                                  ...
                                  localStorage.setItem("t"+this.id+"r3-matches_finished", String(this.num_of_match_finished));
                                  $('#progress_bar').width(Math.round(this.num_of_match_finished * 100 / 8) + "%").attr('aria-valuenow', this.num_of_match_finished);
