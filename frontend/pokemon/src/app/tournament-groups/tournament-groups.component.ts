...
export class TournamentGroupsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.num_group_finished = 0;
    this.id = this.router.snapshot.params.id;
    this.num_of_match_finished = Number(localStorage.getItem("t" + this.id + "g1-matches_finished"));
    $('#progress_bar').width(Math.round(this.num_of_match_finished * 100 / 480) + "%").attr('aria-valuenow', this.num_of_match_finished);
  ...
  find_tournament_by_id(id: Number): void {
    this.service.find_tournament_by_id(id).subscribe(data => {
      ...
      this.service.find_groups_for_tournament(id, 2).subscribe(data => {
        ...
        for (let index in this.groups) {
          ...
          /*if(Number(index) == this.groups.length-1){
            //alert("fde");
            alert(this.last_seen);
            $(document).ready(function () {
              for (let i = 0; i < 32; i++) {
                alert(this.last_seen[String(i)]);
                $("table." + i).hover(function () {
                  this.last_seen[String(i)] = true;
                }, function(){
                  this.last_seen[String(i)] = false;
                })
              }
                            ...
                            this.service.find_main_attack_by_id(main_attack.idAttack).subscribe(data => {
                              ...
                              this.service.find_pokemon_details_by_pokemon_and_type(match.idPokemon1, main_attack2.type).subscribe(data => {
                                ...
                                this.service.update_match(match.id, Math.round(hp_left1), Math.round(hp_left2), res, fast_attack1.id, fast_attack2.id, main_attack1.id, main_attack2.id).subscribe(data => {
                                  ...
                                  localStorage.setItem("t" + this.id + "g1-matches_finished", String(this.num_of_match_finished));
                                  $('#progress_bar').width(Math.round(this.num_of_match_finished * 100 / 480) + "%").attr('aria-valuenow', this.num_of_match_finished);
                                            ...
                                            if ((index + 1) < size) {
                                              ...
                                              this.service.find_pokemon_for_group(match.idGroup).subscribe(data => {
                                                ...
                                                $(document).ready(function () {
                                                  ...
                                                  $("#" + indexInGroup).prop("disabled", true);
                                                ...
                                                if (this.num_of_match_finished == 480) {
                                                  $(document).ready(function () {
  ...
  add_pokemon_to_group(br: number, group_id: number, pokemon_s: PokemonGroup[], index: number, number_of_groups: number, max_id: number, num: number, pok_groups: PokemonGroup[], match_max_id: number): void {
    
    this.service.add_pokemon_to_group(group_id, pokemon_s[br].idPokemon, pokemon_s[br].namePokemon).subscribe(data => {
      //alert(group_id);
      if (data['ok'] == 'no') {
        alert("Something went wrong");
        return;
      ...
      br++;
      //alert(pokemon_s.length + " " +  number_of_groups)
      //alert(br + " " + index);
      if (br == pokemon_s.length  && index == number_of_groups -1) {
        //alert("br" + br + " num gr" + number_of_groups)
        localStorage.setItem("t" + this.id + "g2-matches_finished", "0");
        this.router1.navigate(['/tournament_second_groups/' + this.id]);
        return;
      }
      
      if (br < pokemon_s.length) {
        
        this.add_pokemon_to_group(br, group_id, pokemon_s, index, number_of_groups, max_id, num, pok_groups, match_max_id);
      }
      else{
        
        index++;
        if (index < number_of_groups) {
          //alert(index + " " + number_of_groups);
          this.add_to_group(index, number_of_groups, max_id, num, pok_groups, match_max_id);
        }
      }
    })
  }

  add_to_group(index: number, number_of_groups: number, max_id: number, num: number, pok_groups: PokemonGroup[], match_max_id): void {
    let group_id = max_id++;
    //alert(index);
    this.service.add_new_group(group_id, String(index + 1), this.id, 1).subscribe(data => {
      //alert('tu sam');
      let pokemon_s: PokemonGroup[];
      pokemon_s = new Array;
      for (let j = 0; j < 8; j++) {
      ...
      for (let m = 0; m < 7; m++) {
        for (let n = m + 1; n < 8; n++) {
          ...
          this.service.add_new_match(match_id, group_id, this.id, pokemon_s[m].idPokemon, pokemon_s[n].idPokemon, "g2").subscribe(data => {
            //alert(match_id + " " + group_id + " " + this.id + " " + pokemon_s[m].idPokemon + " " + pokemon_s[n].idPokemon);
            if (data['ok'] == 'no') {
              alert("Something went wrong");
              return;
            }
      ...
      this.add_pokemon_to_group(0, group_id, pokemon_s, index, number_of_groups, max_id, num, pok_groups, match_max_id);
  ...
  next_stage(): void {
    ...
    this.service.find_over_groups_for_tournament(this.id, 2).subscribe(data => {
      groups_ = JSON.parse(JSON.stringify(data));
      //alert(groups_);
      num_groups = groups_.length;
      for (let index in groups_) {
        this.service.find_pokemon_who_passed_for_group(groups_[index].id).subscribe(data => {
          let pok_group: PokemonGroup[];
          pok_group = new Array;
          pok_group = JSON.parse(JSON.stringify(data));
          //alert(pok_group);
          for (let index_i in pok_group) {
            pok_groups.push(pok_group[index_i]);
            if (Number(index_i) == pok_group.length - 1 && Number(index) == groups_.length - 1) {
              //alert("jek");
              let number_of_groups: number;
              let num: number;

              number_of_groups = pok_groups.length / 8;
              ...
              this.service.get_group_max_id().subscribe(data => {
                ...
                this.service.get_match_max_id().subscribe(data => {
                  ...
                  this.add_to_group(0, number_of_groups, max_id, num, pok_groups, match_max_id)
