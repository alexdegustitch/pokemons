...
declare var $: any;
@Component({
  selector: 'app-tournament-quarterfinals',
  templateUrl: './tournament-quarterfinals.component.html',
  styleUrls: ['./tournament-quarterfinals.component.css']
})
export class TournamentQuarterfinalsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.num_of_match_finished = Number(localStorage.getItem("t"+this.id+"q-matches_finished"));
    $('#progress_bar').width(Math.round(this.num_of_match_finished * 100 / 4) + "%").attr('aria-valuenow', this.num_of_match_finished);
  ...
  find_tournament_by_id(id: Number): void {
    this.service.find_tournament_by_id(id).subscribe(data => {
      ...
      this.service.find_matches_by_type(id, "q").subscribe(data => {
                            ...
                            this.service.find_main_attack_by_id(main_attack.idAttack).subscribe(data => {
                              ...
                              this.service.find_pokemon_details_by_pokemon_and_type(match.idPokemon1, main_attack2.type).subscribe(data => {
                                ...
                                this.service.update_match(match.id, Math.round(hp_left1), Math.round(hp_left2), res, fast_attack1.id, fast_attack2.id, main_attack1.id, main_attack2.id).subscribe(data => {
                                  ...
                                  localStorage.setItem("t"+this.id+"q-matches_finished", String(this.num_of_match_finished));
                                  $('#progress_bar').width(Math.round(this.num_of_match_finished * 100 / 4) + "%").attr('aria-valuenow', this.num_of_match_finished);
